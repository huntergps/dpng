odoo.define('l10n_ec_balcon.SalePortalSidebar', function (require) {
'use strict';
...
publicWidget.registry.SalePortalSidebar = PortalSidebar.extend({
    selector: '.o_portal_balcon_sidebar',
    ...
    start: function () {
        ...
        if ($.bbq.getState('allow_payment') === 'yes' && this.$('#o_balcon_portal_paynow').length) {
            this.$('#o_balcon_portal_paynow').trigger('click');
