...
class BalconReport(models.Model):
    _name = "balcon.report"
    _description = "Balcon Analysis Report"
    _auto = False
    _rec_name = 'date'
    _order = 'date desc'
    ...
    order_id = fields.Many2one('balcon.order', 'Order #', readonly=True)
    ...
    def _from_sale(self, from_clause=''):
        from_ = """
                balcon_order_line l
                      right outer join balcon_order s on (s.id=l.order_id)
    ...
    def init(self):
        # self._table = balcon_report
        tools.drop_view_if_exists(self.env.cr, self._table)
        self.env.cr.execute("""CREATE or REPLACE VIEW %s as (%s)""" % (self._table, self._query()))

class BalconOrderReportProforma(models.AbstractModel):
    _name = 'report.balcon.report_saleproforma'
    _description = 'Balcon Proforma Report'

    @api.model
    def _get_report_values(self, docids, data=None):
        docs = self.env['balcon.order'].browse(docids)
        return {
            'doc_ids': docs.ids,
            'doc_model': 'balcon.order',
            'docs': docs,
            'proforma': True
        }
