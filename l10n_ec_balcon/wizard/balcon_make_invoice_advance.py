...
class SaleAdvancePaymentInv(models.TransientModel):
    _name = "l10n_ec_balcon.advance.payment.inv"
    _description = "Balcon Advance Payment Invoice"
    ...
    def _default_product_id(self):
        product_id = self.env['ir.config_parameter'].sudo().get_param('l10n_ec_balcon.default_deposit_product_id')
    ...
    def _default_has_down_payment(self):
        if self._context.get('active_model') == 'balcon.order' and self._context.get('active_id', False):
            ...
            return balcon_order.order_line.filtered(
                lambda balcon_order_line: balcon_order_line.is_downpayment
            )
    ...
    def _prepare_invoice_values(self, order, name, amount, bo_line):
        invoice_vals = {
            ...
            'invoice_line_ids': [(0, 0, {
                ...
                'product_uom_id': bo_line.product_uom.id,
                'tax_ids': [(6, 0, bo_line.tax_id.ids)],
                'balcon_line_ids': [(6, 0, [bo_line.id])],
                'analytic_tag_ids': [(6, 0, bo_line.analytic_tag_ids.ids)],
    ...
    def _create_invoice(self, order, bo_line, amount):
        ...
        invoice_vals = self._prepare_invoice_values(order, name, amount, bo_line)
    ...
    def create_invoices(self):
        ...
        if self.advance_payment_method == 'delivered':
            balcon_orders._create_invoices(final=self.deduct_down_payments)
        else:
            # Create deposit product if necessary
            if not self.product_id:
                vals = self._prepare_deposit_product()
                self.product_id = self.env['product.product'].create(vals)
                self.env['ir.config_parameter'].sudo().set_param('l10n_ec_balcon.default_deposit_product_id', self.product_id.id)

            balcon_line_obj = self.env['balcon.order.line']
            for order in balcon_orders:
                amount, name = self._get_advance_details(order)
                ...
                bo_line = balcon_line_obj.create(so_line_values)
                self._create_invoice(order, bo_line, amount)
