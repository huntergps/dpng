...
class BalconOrderLine(models.Model):
    _name = 'balcon.order.line'
    _description = 'Balcon Order Line'
    ...
    def write(self, values):
        ...
        result = super(BalconOrderLine, self).write(values)
        return result

    ##############################################################################
    country_id = fields.Many2one('res.country', string='Country', required=True, index=True)
    nationality_id = fields.Many2one('res.country', string='Nationality', required=True, index=True)
    vat_type = fields.Selection([
        ('cedula', 'ID Card'),
        ('passport', 'Passport')
        ], string='Identification Type',  default='cedula')
    vat = fields.Char('ID Card')
    partner_name = fields.Char('Complete names')
    ...
    def _default_origin(self):
        return  self.order_id.origin if self.order_id else 'uio'

    origin = fields.Selection([
        ('uio', 'Quito'),
        ('gye', 'Guayaquil'),
        ('other', 'Other')
        ], string='Origin',  default=_default_origin)

    ids_diplomat = fields.Char('Is Diplomat')

    def _default_date_in(self):
        return  self.order_id.date_in if self.order_id else fields.Date.today()

    def _default_date_out(self):
        return  self.order_id.date_out if self.order_id else fields.Date.to_string(datetime.now() + timedelta(2))
    ...
    days_visit = fields.Integer(string='Total Nigths', compute='_compute_days_visit', store=True)
    days_visit_land = fields.Integer(string='Total Land Nigths', compute='_compute_days_visit', store=True)
    days_visit_cruise = fields.Integer(string='Total Cruise Nigths')

    accommodation_type = fields.Selection([
        ('cruise', 'Cruise'),
        ('relative', 'Friend Family Address'),
        ('hotel', 'Hotel')
        ], string='Accommodation',  default='cruise')
    ...
    @api.onchange('days_visit_land')
    def _onchange_days_visit_land(self):
        self.days_visit_cruise = self.days_visit - self.days_visit_cruise if self.days_visit < self.days_visit_cruise else 0
    ...
    def _compute_days_visit(self):
        for line in self:
            if line.date_in and line.date_out:
                days_visit = (line.date_in-line.date_out).days or 0
                line.days_visit = days_visit
            else:
                line.days_visit = 0
            days_visit_land = line.days_visit_land
            line.days_visit_cruise= days_visit if line.days_visit_land else days_visit_land


    @api.onchange('vat')
    def onchange_vat(self):
        self.consultar_cedula()

    @api.onchange('country_id')
    def onchange_country_id(self):
        nationality_id = self.nationality_id
        self.nationality_id = self.country_id if not self.nationality_id else nationality_id

    @api.onchange('nationality_id')
    def onchange_nationality_id(self):
        self.vat_type='cedula' if self.nationality_id.code=='EC' else 'passport'
        self.partner_name=''
        self.birth_date= False
        self.email = ''
        self.product_id=False

    domain_product_ids = fields.Char(compute='_compute_domain_product_ids')

    def consultar_cedula(self):
        if self.vat and len(self.vat)==10:
            URL ="http://facturas.galapagos.gob.ec/demon.io/cedula/%s"%self.vat
            data = requests.get(URL)
            if data:
                data= data.json()
                if data['Retorno']=='FAIL':
                    self.partner_name=''
                    self.marital_status=''
                    self.gender=''
                    self.birth_date=False
                    # UserError(data['razon'])
                else:
                    self.partner_name=data['nombre']
                    self.marital_status=marital_status_registro_civil[data['estadoCivil']]
                    self.gender=gender_registro_civil[data['sexo']]
                    self.birth_date=datetime.strptime(data['fechaNacimiento'], '%d/%m/%Y')
                    self._onchange_age()
                    self._compute_domain_product_ids()
            else:
                raise UserError('Error al consultar al Registro Civil')


    @api.depends('birth_date','nationality_id')
    def _compute_domain_product_ids(self):
        for rec in self:
            domain=[('balcon_ok', '=', True), '|', ('company_id', '=', False), ('company_id', '=', rec.order_id.company_id.id),('categ_id','=',rec.order_id.categ_id.id)]
            if rec.age_years>0:
                if rec.nationality_id.code=='EC':
                    domain.append(('extranjero','=',False))
                else:
                    domain.append(('extranjero','=',True))

                if rec.age_years<19:
                    domain.append(('menor_edad','=',True))
                else:
                    domain.append(('menor_edad','=',False))
                    if rec.age_years>64:
                        domain.append(('tercera_edad','=',True))
                    else:
                        domain.append(('tercera_edad','=',False))

            rec.domain_product_ids =  json.dumps(domain)
    ##############################################################################

    order_id = fields.Many2one('balcon.order', string='Order Reference', required=True, ondelete='cascade', index=True, copy=False)
    name = fields.Text(string='Description', required=True)
    sequence = fields.Integer(string='Sequence', default=10)

    invoice_lines = fields.Many2many('account.move.line', 'balcon_order_line_invoice_rel', 'order_line_id', 'invoice_line_id', string='Invoice Lines', copy=False)
    ...
    product_id = fields.Many2one(
        'product.product', string='Product', domain="[('balcon_ok', '=', True), '|', ('company_id', '=', False), ('company_id', '=', company_id)]",
        change_default=True, ondelete='restrict', check_company=True)  # Unrequired company
    product_template_id = fields.Many2one(
        'product.template', string='Product Template',
        related="product_id.product_tmpl_id", domain=[('balcon_ok', '=', True)])
    ...
    product_custom_attribute_value_ids = fields.One2many('product.attribute.custom.value', 'balcon_order_line_id', string="Custom Values", copy=True)
    ...
    analytic_line_ids = fields.One2many('account.analytic.line', 'bo_line', string="Analytic lines")
    ...
    def _compute_qty_delivered_method(self):
        """ Sale module compute delivered qty for product [('type', 'in', ['consu']), ('service_type', '=', 'manual')]
            ...
            This is true when only sale is installed: balcon_stock redifine the behavior for 'consu' type,
            and balcon_timesheet implements the behavior of 'service' + service_type=timesheet.
        """
        for line in self:
            if line.is_expense:
    ...
    @api.depends('qty_delivered_method', 'qty_delivered_manual', 'analytic_line_ids.bo_line', 'analytic_line_ids.unit_amount', 'analytic_line_ids.product_uom_id')
    def _compute_qty_delivered(self):
        ...
        for bo_line in lines_by_analytic:
            bo_line.qty_delivered = mapping.get(bo_line.id or bo_line._origin.id, 0.0)
    ...
    def _get_delivered_quantity_by_analytic(self, additional_domain):
        ...
        domain = expression.AND([[('bo_line', 'in', self.ids)], additional_domain])
        data = self.env['account.analytic.line'].read_group(
            domain,
            ['bo_line', 'unit_amount', 'product_uom_id'], ['product_uom_id', 'bo_line'], lazy=False
        )

        # convert uom and sum all unit_amount of analytic lines to get the delivered qty of SO lines
        # browse so lines and product uoms here to make them share the same prefetch
        lines = self.browse([item['bo_line'][0] for item in data])
        ...
        for item in data:
            if not item['product_uom_id']:
                continue
            so_line_id = item['bo_line'][0]
            bo_line = lines_map[so_line_id]
            result.setdefault(so_line_id, 0.0)
            uom = product_uom_map.get(item['product_uom_id'][0])
            if bo_line.product_uom.category_id == uom.category_id:
                qty = uom._compute_quantity(item['unit_amount'], bo_line.product_uom, rounding_method='HALF-UP')
            else:
    ...
    def _get_display_price(self, product):
        # TO DO: move me in master/saas-16 on balcon.order
    ...
    def product_id_change(self):
        ...
        vals.update(name=self.get_balcon_order_line_multiline_description_sale(product))
        ...
        if product.balcon_line_warn != 'no-message':
            if product.balcon_line_warn == 'block':
                self.product_id = False

            return {
                'warning': {
                    'title': _("Warning for %s", product.name),
                    'message': product.balcon_line_warn_msg,
                }
            }
    ...
    def name_get(self):
        result = []
        for bo_line in self.sudo():
            name = '%s - %s' % (bo_line.order_id.name, bo_line.name and bo_line.name.split('\n')[0] or bo_line.product_id.name)
            if bo_line.order_partner_id.ref:
                name = '%s (%s)' % (name, bo_line.order_partner_id.ref)
            result.append((bo_line.id, name))
        return result
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        return super(BalconOrderLine, self)._name_search(name, args=args, operator=operator, limit=limit, name_get_uid=name_get_uid)
    ...
    def _check_line_unlink(self):
        ...
        :rtype: recordset balcon.order.line
    ...
    def get_balcon_order_line_multiline_description_sale(self, product):
        ...
        """
        return product.get_product_multiline_description_sale() + self._get_balcon_order_line_multiline_description_variants()

    def _get_balcon_order_line_multiline_description_variants(self):
