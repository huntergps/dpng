...
class BalconOrder(models.Model):
    _name = "balcon.order"
    _inherit = ['portal.mixin', 'mail.thread', 'mail.activity.mixin', 'utm.mixin']
    _description = "Balcon Orders"
    _order = 'date_order desc, id desc'
    _check_company_auto = True

    def _default_validity_date(self):
        if self.env['ir.config_parameter'].sudo().get_param('balcon.use_quotation_validity_days'):
            days = self.env.company.quotation_validity_days
            if days > 0:
                return fields.Date.to_string(datetime.now() + timedelta(days))
        return False

    def _default_date_in(self):
    ...
    #     return True #self.env.company.portal_confirmation_pay

    def _default_validity_date(self):
        if self.env['ir.config_parameter'].sudo().get_param('l10n_ec_balcon.use_balcon_order_validity_days'):
            days = self.env.company.balcon_order_validity_days
            if days > 0:
                return fields.Date.to_string(datetime.now() + timedelta(days))
        return False
    ...
    def _get_invoice_status(self):
        ...
        line_invoice_status_all = [
            ...
            for d in self.env['balcon.order.line'].read_group([
    ...
    def get_empty_list_help(self, help):
        ...
        return super(BalconOrder, self).get_empty_list_help(help)
        ...
        if operator == 'in' and value:
            self.env.cr.execute("""
                SELECT array_agg(so.id)
                    FROM balcon_order so
                    JOIN balcon_order_line sol ON sol.order_id = so.id
                    JOIN balcon_order_line_invoice_rel soli_rel ON soli_rel.order_line_id = sol.id
    ...
    def action_register_payment(self):
        if self.payment_registered_amount>=self.amount_total:
            raise UserError('Ya ha registrado transacciones de Pagos por el monto total de la Orden de Servicio\nDebe esperar la revision y aprobacion')
        else:
            return {
                'name': _('>Registrar Pagos'),
                'view_mode': 'form',
                'res_model': 'balcon.bank.payment',
                'view_id': self.env.ref('l10n_ec_balcon.balcon_order_bank_payment_view_form').id,
    ...
    partner_id = fields.Many2one(
        ...
        default=lambda rec: rec._default_customer_from_user())

    group_balcon_external = fields.Boolean('group_balcon_external',compute='_get_actualizar_grupos_seguridad')
    group_balcon_salesman = fields.Boolean('group_balcon_salesman',compute='_get_actualizar_grupos_seguridad')
    group_balcon_manager = fields.Boolean('group_balcon_manager',compute='_get_actualizar_grupos_seguridad')

    categ_id = fields.Many2one(
        'product.category', 'Categoria del Servicio',readonly=True,
        states={'draft': [('readonly', False)]},
        change_default=True)
    ...
    payment_type = fields.Selection([
        ...
        readonly=True,
        states={'draft': [('readonly', False)], 'balcon': [('readonly', False)]},
        )


    @api.onchange('payment_type')
    def onchange_payment_type(self):
        self.require_payment = self.payment_type=='online'

    origin = fields.Selection([
        ('uio', 'Quito'),
        ('gye', 'Guayaquil'),
        ('other', 'Other')
        ], string='Origin',  default='uio',
        readonly=True,
        states={'draft': [('readonly', False)]})

    date_in = fields.Date(string='Entry Date', default=_default_date_in,
    readonly=True,
    states={'draft': [('readonly', False)]})
    date_out = fields.Date(string='Out Date', default=_default_date_out,
    readonly=True,
    states={'draft': [('readonly', False)]})
    days_visit = fields.Integer(string='Total Days', compute='_compute_days_visit', store=True)

    @api.depends('date_in', 'date_out')
    def _compute_days_visit(self):
        for order in self:
            days_visit = 0
            if order.date_in and order.date_out:
                days_visit = (order.date_out-order.date_in).days or 0
            order.days_visit=days_visit

    accommodation_type = fields.Selection([
        ('cruise', 'Cruise'),
        ('relative', 'Friend Family Address'),
        ('hotel', 'Hotel')
        ], string='Accommodation',  default='cruise',readonly=True,
        states={'draft': [('readonly', False)]},)


    @api.onchange('state','user_id','partner_id','date_order','payment_term_id','amount_total')
    def _get_actualizar_grupos_seguridad(self):
        for order in self:
            group_balcon_external = self.user_has_groups('l10n_ec_balcon.group_balcon_external')
            group_balcon_salesman = self.user_has_groups('l10n_ec_balcon.group_balcon_salesman')
            group_balcon_manager = self.user_has_groups('l10n_ec_balcon.group_balcon_manager')
            order.group_balcon_external = group_balcon_external
            order.group_balcon_salesman = group_balcon_salesman
            order.group_balcon_manager = group_balcon_manager

    @api.model
    def _default_customer_from_user(self):
        # ICPSudo = self.env['ir.config_parameter'].sudo()
        # sale_customer_id = literal_eval(ICPSudo.get_param('sale_customer_id', default='False'))
        sale_customer_id = False
        if self.user_has_groups('l10n_ec_balcon.group_balcon_external'):
            sale_customer_id = self.env.user.partner_id.id
        return sale_customer_id

    @api.depends('order_line.price_total','transaction_ids','authorized_transaction_ids')
    def _amount_payment_transactions(self):
        for order in self:
            payment_registered_amount = payment_authorized_amount = amount_total_balance = 0.0
            for line in order.transaction_ids:
                payment_registered_amount += line.amount
            for line in order.authorized_transaction_ids:
                payment_authorized_amount += line.amount
            amount_total_balance = order.amount_total-payment_authorized_amount
            order.update({
                'payment_registered_amount': payment_registered_amount,
                'payment_authorized_amount': payment_authorized_amount,
                'amount_total_balance':amount_total_balance,
            })

    payment_registered_amount = fields.Monetary(string='Payment Registered Amount', store=True, readonly=True, compute='_amount_payment_transactions')
    payment_authorized_amount = fields.Monetary(string='Payment Authorized Amount', store=True, readonly=True, compute='_amount_payment_transactions')
    amount_total_balance = fields.Monetary(string='Payment Balance', store=True, readonly=True, compute='_amount_payment_transactions')
    ...
    order_line = fields.One2many('balcon.order.line', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True, auto_join=True)
    ...
    transaction_ids = fields.Many2many('payment.transaction', 'balcon_order_transaction_rel', 'balcon_order_id', 'transaction_id',
    ...
    def _compute_access_url(self):
        super(BalconOrder, self)._compute_access_url()
        for order in self:
    ...
    def _compute_expected_date(self):
        """ For service and consumable, we only take the min dates. This method is extended in balcon_stock to
    ...
    def _compute_tax_totals_json(self):
        def compute_taxes(order_line):
            ...
            return order_line.tax_id._origin.compute_all(price, order.currency_id, order_line.product_uom_qty, product=order_line.product_id, partner=order.partner_id)
    ...
    def _compute_type_name(self):
        for record in self:
            record.type_name = _('Quotation') if record.state in ('draft', 'sent', 'cancel') else _('Balcon Order')
    ...
    def validate_taxes_on_balcon_order(self):
    ...
    def _track_subtype(self, init_values):
        ...
        if 'state' in init_values and self.state == 'sale':
            return self.env.ref('l10n_ec_balcon.mt_balcon_order_confirmed')
        elif 'state' in init_values and self.state == 'sent':
            return self.env.ref('l10n_ec_balcon.mt_balcon_order_sent')
        return super(BalconOrder, self)._track_subtype(init_values)
    ...
    def _onchange_partner_id_warning(self):
        ...
        if partner.balcon_warn == 'no-message' and partner.parent_id:
            partner = partner.parent_id

        if partner.balcon_warn and partner.balcon_warn != 'no-message':
            # Block if partner only has warning but parent company is blocked
            if partner.balcon_warn != 'block' and partner.parent_id and partner.parent_id.balcon_warn == 'block':
                partner = partner.parent_id

            if partner.balcon_warn == 'block':
                self.update({'partner_id': False, 'partner_invoice_id': False, 'pricelist_id': False})

            return {
                'warning': {
                    'title': _("Warning for %s", partner.name),
                    'message': partner.balcon_warn_msg,
                }
            }
    ...
    def create(self, vals):
        ...
        if vals.get('name', _('New')) == _('New'):
            ...
            vals['name'] = self.env['ir.sequence'].next_by_code('balcon.order', sequence_date=seq_date) or _('New')
        ...
        result = super(BalconOrder, self).create(vals)
        return result
    ...
    def copy_data(self, default=None):
        ...
        return super(BalconOrder, self).copy_data(default)

    def name_get(self):
        if self._context.get('balcon_show_partner_name'):
            res = []
            for order in self:
                name = order.name
            ...
            return res
        return super(BalconOrder, self).name_get()
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        return super(BalconOrder, self)._name_search(name, args=args, operator=operator, limit=limit, name_get_uid=name_get_uid)

    def _create_upsell_activity(self):
        self and self.activity_unlink(['l10n_ec_balcon.mail_act_balcon_upsell'])
        for order in self:
            ...
            order.activity_schedule(
                'l10n_ec_balcon.mail_act_balcon_upsell',
    ...
    def _get_invoiceable_lines(self, final=False):
        ...
        return self.env['balcon.order.line'].browse(invoiceable_line_ids + down_payment_line_ids)
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        for order in self:
            ...
            down_payments = order.env['balcon.order.line']
        ...
        if len(invoice_vals_list) < len(self):
            BalconOrderLine = self.env['balcon.order.line']
            for invoice in invoice_vals_list:
                sequence = 1
                for line in invoice['invoice_line_ids']:
                    line[2]['sequence'] = BalconOrderLine._get_invoice_line_sequence(new=sequence, old=line[2]['sequence'])
                    sequence += 1
        ...
        for move in moves:
            move.message_post_with_view('mail.message_origin_link',
                values={'self': move, 'origin': move.line_ids.mapped('balcon_line_ids.order_id')},
    ...
    def action_cancel(self):
        ...
        if cancel_warning:
            return {
                'name': _('Cancel Balcon Order'),
                'view_mode': 'form',
                'res_model': 'balcon.order.cancel',
                'view_id': self.env.ref('l10n_ec_balcon.balcon_order_cancel_view_form').id,
                'type': 'ir.actions.act_window',
                'context': {'default_order_id': self.id},
                'target': 'new'
            }
    ...
    def _find_mail_template(self, force_confirmation_template=False):
        ...
        if force_confirmation_template or (self.state == 'sale' and not self.env.context.get('proforma', False)):
            template_id = int(self.env['ir.config_parameter'].sudo().get_param('l10n_ec_balcon.default_confirmation_template'))
            template_id = self.env['mail.template'].search([('id', '=', template_id)]).id
            if not template_id:
                template_id = self.env['ir.model.data']._xmlid_to_res_id('l10n_ec_balcon.mail_template_balcon_confirmation', raise_if_not_found=False)
        if not template_id:
            template_id = self.env['ir.model.data']._xmlid_to_res_id('l10n_ec_balcon.email_template_edi_sale', raise_if_not_found=False)
    ...
    def message_post(self, **kwargs):
        ...
        return super(BalconOrder, self.with_context(mail_post_autofollow=self.env.context.get('mail_post_autofollow', True))).message_post(**kwargs)
    ...
    def _send_balcon_order_confirmation_mail(self):
        if self.env.su:
    ...
    def _action_confirm(self):
        """ Implementation of additionnal mecanism of Balcon Order confirmation.
    ...
    def action_confirm(self):
        ...
        if self.env.user.has_group('l10n_ec_balcon.group_auto_done_setting'):
            self.action_done()
        return True
    ...
    def _notify_get_groups(self, msg_vals=None):
        ...
        in l10n_ec_balcon. Customer and portal group have probably no right to see
        the document so they don't have the access button. """
        groups = super(BalconOrder, self)._notify_get_groups(msg_vals=msg_vals)
    ...
    def _get_portal_return_action(self):
        ...
        self.ensure_one()
        return self.env.ref('l10n_ec_balcon.action_quotations_with_onboarding')
