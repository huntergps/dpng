...
class ProductTemplate(models.Model):
    ...
    balcon_ok = fields.Boolean('Can be Sold', default=True)
    ...
    service_type = fields.Selection([('manual', 'Manually set quantities on order')], string='Track Service',
        ...
        default='manual')
    balcon_line_warn = fields.Selection(WARNING_MESSAGE, 'Balcon Order Line', help=WARNING_HELP, required=True, default="no-message")
    balcon_line_warn_msg = fields.Text('Message for Balcon Order Line')
    ...
    @api.onchange('balcon_ok')
    def _change_balcon_ok(self):
        if not self.balcon_ok:
            self.expense_policy = 'no'

    @api.depends('product_variant_ids.balcon_count')
    def _compute_balcon_count(self):
        for product in self:
            product.balcon_count = float_round(sum([p.balcon_count for p in product.with_context(active_test=False).product_variant_ids]), precision_rounding=product.uom_id.rounding)


    @api.constrains('company_id')
    def _check_balcon_product_company(self):
        ...
        if target_company:  # don't prevent writing `False`, should always work
            ...
            so_lines = self.env['balcon.order.line'].sudo().search_read([('product_id', 'in', product_ids), ('company_id', '!=', target_company.id)], fields=['id', 'product_id'])
    ...
    def get_import_templates(self):
        ...
        if self.env.context.get('balcon_multi_pricelist_product_template'):
